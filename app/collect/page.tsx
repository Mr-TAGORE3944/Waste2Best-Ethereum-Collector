...
export default function CollectPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      try {
        // Clean up the response to remove any markdown or unwanted formatting
        const cleanedText = text.replace(/```json|```/g, "").trim();

        // Try to parse the cleaned JSON text
        const parsedResult = JSON.parse(cleanedText);

        // Set the verification result based on the parsed response
        ...
        // Set the verification status to success
        setVerificationStatus("success");

        // Perform further checks and actions based on the parsed result
        if (
          ...
          // Randomly generate a reward between 10 and 59 tokens
          const earnedReward = Math.floor(Math.random() * 50) + 10;

          // Save the earned reward
          await saveReward(user.id, earnedReward);

          // Save the collected waste data
        ...
        // Log error and set verification status to failure
